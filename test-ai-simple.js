...
async function testAIBot() {
  ...
  if (!API_URL) {
    console.log('📡 请输入 Render 后端 URL')
    console.log('   例如: https://your-app.onrender.com')
    console.log('')
    let url = await question('后端 URL: ')
  ...
  try {
    ...
    if (error.response) {
      ...
      if (error.response.status === 401) {
        ...
        console.error('')
        console.error('💡 当前账号可能不是管理员，请使用管理员账号')
      } else if (error.response.status === 404) {
        console.error('')
        console.error('💡 API URL 可能不正确，请检查后端URL')
      }
    ...
    if (error.code === 'ECONNREFUSED') {
      console.error('')
      console.error('💡 无法连接到服务器，请检查服务器是否正在运行')
    }
