...
export default function AdminDashboard() {
  ...
  const fetchData = async (customStartDate = null, customEndDate = null) => {
    try {
      ...
      // WebSocket for admin notifications (new messages, failed jobs)
      try {
        const token = localStorage.getItem('glowlisting_token')
        if (token) {
          const wsUrl = (API_URL || '').replace(/^http/, 'ws')
          const socket = new WebSocket(`${wsUrl.replace(/\/$/, '')}/ws?token=${token}`)
          socket.onopen = () => console.log('WS connected (admin)')
          socket.onmessage = (event) => {
            try {
              const data = JSON.parse(event.data)
              if (['message_new', 'message_reply', 'job_failed'].includes(data.type)) {
          ...
          socket.onclose = () => console.log('WS closed (admin)')
          setWs(socket)
        }
      } catch (e) {
